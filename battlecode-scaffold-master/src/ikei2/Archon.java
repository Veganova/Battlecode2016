...
public class Archon {
	...
	static int collectQuota =0;
	static int buildQuota = 0;
	static int broadcastedMessages =0;
	...
	public static void repeat() throws GameActionException
	{
		...
		else
		{
			...
			if(rc.senseHostileRobots(rc.getLocation(), rc.getType().sensorRadiusSquared).length>0 &&
					!(rc.senseHostileRobots(rc.getLocation(), rc.getType().sensorRadiusSquared)[0].type==RobotType.ZOMBIEDEN))
			{
			...
			else
			{
				...
				recieveBroadCast();
				if(broadcastedMessages<19 && rc.getRoundNum()%20==4)
				{
...
//	}
	...
	{
		if(keyLocations.get(2).size()>0 && keyLocations.get(2).get(0).distanceSquaredTo(rc.getLocation())<500)
		{
//		when you are there, watching your soldiers take the dens down, keep building
			if(rc.getLocation().distanceSquaredTo(keyLocations.get(2).get(0))<8)
			{
				howtobuild(true);
				timenearden++;
	...
	public static void selfSense() throws GameActionException
	{
		...
		rc.setIndicatorString(0, "0: " + densArray.length + "1: " + dens.size());
			...
			for(int i=0;i<messages.length;i++){//go through all the messages
				if(messages[i].getTeam()==rc.getTeam())//same team ... Perhaps also include if the message if form a scou
				{
					if(messages[i].getMessage()[0]<1000)
					{
							...
							if(broadcastedMessages <19 && messages[i].getMessage()[0]==-2)//den location
							{
