...
public class Attack {
    ...
    public static MapLocation attack(){
        ...
        if(rc.getType().canAttack()&&rc.isWeaponReady()){                              //if there are more friendlies than hostiles nearby
            ...
            {//if it's a unit that can attack
                try {
                    if(rc.getType().canAttack()&&rc.isWeaponReady()&&rc.getType().attackRadiusSquared>rc.getLocation().distanceSquaredTo(hostiles[0].location)){
       ...
       // if(!returnhome(friendlies, 2))
        if(rc.getType()!=RobotType.TURRET)
        {
        	...
        	else if (!rc.getType().canAttack() && hostiles.length>friendlies.length){
        		return kite(hostiles, friendlies);
        	}
        }
        return null;
    ...
    public static MapLocation kite(RobotInfo[] hostiles, RobotInfo[] allies){
        ...
        if((rc.getType()!=RobotType.ARCHON && islow && rc.isInfected()))// || threatlevel<10)
